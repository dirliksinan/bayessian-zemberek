...
public class BayessianAnalyzer {
	...
	/**
	 * score Overloaded function Just takes the path of the file which will be
	 * identified with the help of the other overloaded function of score return
	 * a double score value of the file
	 * 
	 * @param String
	 *            -filename
	 * @return double
	...
	public double score(StringBuffer stringBuffer) {
		...
		Map<Integer, String> incelenen_kelimeler = ZemberekUtil
				.generateDocumentWordRootCountMap(stringBuffer);
		...
		while (iter.hasNext()) {
			...
			if ((!bayessianDB.getClassifiedWordMap().containsKey(word))
					&& (!bayessianDB.getNonclassifiedWordMap()
							.containsKey(word)))
				continue;
			else {
				if (bayessianDB.getClassifiedWordMap().containsKey(word)) {
					wordProbInClassified = bayessianDB.getClassifiedWordMap()
							.get(word); // database deki kelimenin gizli
										// dosyadaki oranı
					classifiedFileProb = bayessianDB.getClassifiedFileNumber()
							/ (bayessianDB.getClassifiedFileNumber() + bayessianDB
									.getNonclassifiedFileNumber()); // database
																	// de ki
																	// gizli
																	// dosya
																	// oranı
				...
				if (bayessianDB.getNonclassifiedWordMap().containsKey(word)) {
					wordProbInNonclassified = bayessianDB
							.getNonclassifiedWordMap().get(word);
					nonClassifiedFileProb = bayessianDB
							.getNonclassifiedFileNumber()
							/ (bayessianDB.getClassifiedFileNumber() + bayessianDB
									.getNonclassifiedFileNumber());
				} else {
				...
				Bayesianprob += (wordProbInClassified * classifiedFileProb)
						/ ((wordProbInClassified * classifiedFileProb) + (wordProbInNonclassified * nonClassifiedFileProb));
				wordnumber++;
			}
	 ...
	 * @param dbFileCount
	 */
	public void updateDbMap(Map<String, Double> dbWordScoreMap,
			Map<String, Double> documentWordCountMap, Double dbFileCount) {
		Iterator<String> dwordCMap_iter = documentWordCountMap.keySet()
				.iterator();
		...
		while (dbWordSMap_iter.hasNext()) {
			...
			double value = dbWordScoreMap.get(word).doubleValue()
					* updateFactor;
		...
		if (dbFileCount == 0) {
			while (dwordCMap_iter.hasNext()) {
				...
				if (dbWordScoreMap.containsKey(word)) {
					double value = dbWordScoreMap.get(word).doubleValue()
							+ documentWordCountMap.get(word).doubleValue();
					dbWordScoreMap.put(word, value);
				} else {
					double value = ((Double) documentWordCountMap.get(word))
							.doubleValue();
					dbWordScoreMap.put(word, value);
				}
			}
			...
			while (dwordCMap_iter.hasNext()) {
				...
				if (dbWordScoreMap.containsKey(word)) {
					...
					Double sayi = ((Double) documentWordCountMap.get(word))
							.doubleValue() * updateFactor;
	...
	/**
	 * An overloaded version of trainNonclassified function. Just finds the
	 * content of the files of the given paths and put the content in a
	 * StringBuffer. Finally it calls the other overloaded version.
	 * 
	 * @param String
	 * @throws FileNotFoundException
	 */
	public void trainNonclassified(String filename)
			throws FileNotFoundException {
	...
	/**
	 * It finds the roots and the frequency of the words in a non-classified
	 * file which will be added to database and out the data in a TreeMap
	 * 
	 * @param stringBuffer
	 */
	public void trainNonclassified(StringBuffer stringBuffer) {
		Map<Integer, String> documentWordRootMap = ZemberekUtil
				.generateDocumentWordRootCountMap(stringBuffer);
		...
		while (temp_iter.hasNext()) {
			...
			if (!documentWordProbMap.containsKey(word)) {
				documentWordProbMap.put(word,
						findWordCountInDocument(word, documentWordRootMap));
			}
		}
		updateDbMap(bayessianDB.getNonclassifiedWordMap(), documentWordProbMap,
				bayessianDB.getNonclassifiedFileNumber());
		bayessianDB.setNonclassifiedFileNumber(bayessianDB
				.getNonclassifiedFileNumber() + 1);
	}

	/**
	 * An overloaded version of trainClassified function. Just finds the content
	 * of the files of the given paths and put the content in a StringBuffer.
	...
	/**
	 * It finds the roots and the frequency of the words in a classified file
	 * which will be added to database and out the data in a TreeMap
	 * 
	 * @param stringBuffer
	 */
	public void trainClassified(StringBuffer stringBuffer) {
		...
		updateDbMap(bayessianDB.getClassifiedWordMap(), documentWordProbMap,
				bayessianDB.getClassifiedFileNumber());
		bayessianDB.setClassifiedFileNumber(bayessianDB
				.getClassifiedFileNumber() + 1);
	}
	 ...
	 * @return
	 */
	public Double findWordCountInDocument(String documentWord,
			Map<Integer, String> documentWordMap) {
